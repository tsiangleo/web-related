...
//            FilesUtil.getProperty("auth_token_name", FilesConstants.X_AUTH_TOKEN)).getValue();
        return getResponseHeader(
                FilesConstants.X_AUTH_TOKEN).getValue();
        //        if (authToken == null)
        //        {
        //            parseAuthResponse();
        //        }
    ...
    private void parseAuthResponse()
    {
        try
        {
            ...
            for (int i = 0; i < filesRegions.size(); i++)
            {
                ...
                Boolean v1Default = filesRegion.containsKey("v1Default") ? (Boolean) filesRegion
                    .get("v1Default") : Boolean.FALSE;
